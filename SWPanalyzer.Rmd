...
This script helps you to manipulate and analyze raw data from electrophysiological recordings of plants made with the software LabScribe. It analyzes SWP traces and determines different parameters on which statistics can be run. These parameters are:
  ...
  hyperpol.amplitude1 - Hyperpolarization amplitude from first baseline to peak hyperpol 
  depol.amplitude     - Depolarization amplitude from peak hyperpol to peak depol.
  repol.amplitude     - Repolarization amplitude from second baseline to peak repolarization.
  max.hyperpol.rate   - Max peak of hyperpolarizing slope.
  min.depol.rate      - Minimum peak of slope in the depolarization phase.
  max.hyperpol.rate   - Maximum peak of slope in the first hyperpol phase.
  bl2                 - Final baseline after SWP is finished
  depDur              - Time between the following two time points: 
                        I) @ time of min.depol.rate
                        II) @ time of middle voltage between peak repolarization and peak depolarization
  hyperpol.duration   - Duration of first hyperpolarization from time @ max.hyperpol.rate to time @ min.depol.rate.
  
    
    INPUT:
 .txt files in a folder with the following structure.
Data for one experiment of recordings regardless of the amount of genotypes analyzed. All the traces from plants that grew together and were recorded together are considered to be one experiment.
...
A folder with a name of choice inputed in step 2.1
In this folder 
  - a >genotype<->date< folder with all the traces corresponding to that combination. Each image is saved as the plant#.L##
  - >genotype<.csv file with the raw data organized as a data frame to use with R or any other coding language.
  - >genotype<Summary.csv file summarizing for every leaf of every plant all the parameters calculated. Each parameter is a column.
  - >date<-basicStats.csv file with the Shapiro-Wilk normality test, the Mann-Whitney U test and the Kruskal-Wallis test for each parameter of each leaf of each genotype. Both non-parametric tests are compared to Col0 or any other group stated in step 2.1.
  - >date-parameter<.pdf files. One for each parameter - violin plot with all the point separated by genotype and leaf.
  - Mean->genotype-L##<.pdf files. One for each leaf of each genotype. The graphical mean of all traces ± SEM. 
...
The analysis is divided in 4 steps. Run them in order. There is a checkpoint between steps; this means that it is better if you don't close the program before finishing all the sub-steps (3.1,3.2,3.3) in a step (3).  
...
## New things to do
- include the duration of hyperpol peak in Parameters.
...
foldername = "tst"  # Replace the text in "" with the name you want your folder to have.
...
# 5 Plot mean after manual data selection
...
Under contstruction. Plot x - the time after treatment. y - SWP parameters
```{r}
filename = "Col0.CNQX1mM.csv" 
data = read.csv( filename )
SummaryData$treatmentTime = 0
for (i in 1:nrow(SummaryData)){
  daytime = data %>% filter(Date==SummaryData$Date[i] & id==SummaryData$id[i] ) %>%
    select(DayTime)
  daytime = daytime$DayTime[1]
  SummaryData$treatmentTime[i] = daytime
}
write.csv(SummaryData, "Col0.CNQX1mMSummary2.csv", row.names = FALSE)

SummaryData = read.csv("FullSummaryMin.csv")


plot <- SummaryData %>% filter(Leaf=="L13") %>%
  ggplot(aes(post.treatment,min.depol.rate, color=Genotype)) +
  geom_point() +
  xlab("time after treatment (min)")
print(plot)
  
ggsave(filename = "depDur", plot)
