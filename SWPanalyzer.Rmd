...
Follow these steps:
  1. Click on create a project on the top left of this window to create a new project. This will create a folder that will be your "home" directory. In this home directory you put the raw files to be analyzed . 

  2. In the gear instructions at the top of the window select "Chunk output in console" instead of "Chunk output inline". This will show all the steps of the process properly in the console.
  
  3. Paste a copy of the exported folders with the .txt data from one batch of experiments into the home folder you just created.
...
for ( g in unique(SummaryData$Genotype) ){ # pick genotype
  for(l in unique(dplyr::filter(SummaryData, Genotype==g)$Leaf) ){ # pick leaf for genotype g.
    for( v in which( names(SummaryData)%in%Parameters ) ) { # loop through columns
      ...
      StatsData = rbind(StatsData, data.frame(g,l,names(SummaryData)[v],average,sem, n, Shapiro, WM ))
    }
  } 
}
names(StatsData) <- c("Genotype","Leaf","Variable", "Mean", "SEM", "N", "ShapiroTest", "Whitney-Mann")
