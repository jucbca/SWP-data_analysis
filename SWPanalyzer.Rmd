...
DataExtraction <- function(data, date){ 
  ...
  # determine the leaf identity (8, 13)
    if( L08.exist == TRUE && L13.exist == FALSE ) {
        ...
        info1 = names(data)[  grep(leaf1 , names(data))  ]
    } else if ( L08.exist == TRUE && L13.exist == TRUE ) {# if there is both L08 and 13.
        leaf1 = paste("L08.", n, sep = "")
     ...
     for( stimi in unique(subSet1$Stim) ) { # pick one stimulus form
      ...
      for( IDi in unique(subSet2$id) ) { # pick one id
        ...
        for ( leafi in unique(subSet3$Leaf) ){ # pick a leaf
          ...
          answer <- readline (prompt="Trim trace? Indicate time (s) at which to cut + ENTER, just ENTER to skip trimming, or >n<+ENTER for skipping & erasing trace  ")
          if(nchar(answer)>0){
            if(answer == "n"){
  ...
  #### Save or not
          if(abs(SWPvalues$depol.amplitude) < 10){
            ...
            if(answer == "y"){
              ...
              SWPvalues$hyperpol.amplitude = NA
              SWPvalues$hyperpolFrame = NA
              SWPvalues$hyperpol.slope = NA
              SWPvalues$halfmax.depol.frame = NA
              SWPvalues$depol.slope = NA
              SWPvalues$depol.amplitude = NA
              SWPvalues$depolFrame = NA
              SWPvalues$hyperpol.amplitude.2nd = NA
              SWPvalues$depol.duration = NA
              SWPvalues$hyperpol.duration = NA
          ...
          while ( is.null(answer)==FALSE ){ #answer != "y" || answer != "n"
            ...
            if (answer == "y") {
              ...
              id = IDi
              Leaf = leafi
              if( length(grep("FAIL",plotTitle)) != 0 ){
                Stim = "FAIL"
              }
              # save data
...
2.2. Run this to extract the data and save it in a folder with the name previously given. Ignore the errors that are shown at the end. This is due to the F1 marks in the traces with which you marked the wounding event. 

Troubleshooting: If this stops with an error instead of a success message, the last file printed in the console does not have the proper formatting. Check that column names fit the INPUT conditions above.
...
TO RUN THIS WHOLE SECTION A SECOND TIME, READ HERE.  Place only the ">genotype<.csv" files in the batch folder and reset them with this chunk.
```{r}
...
My_Theme =  theme(
  axis.text.x = element_text(color = "black", size = 10, angle = 300, hjust = 0, vjust = 1, face = "plain"),
...
```{r}
setwd(batchFolder)
# Mean ± SEM trace for all the traces
My_Theme =  theme(
...
input - >genotype<.csv files in the home folder. 
      - INPUT BY HAND THE DATES AND IDs to be included. Type them in the "plot.ids" variable.
...
# example: plot.ids = list( "220125" = c(3,6,12,15,17,23,19,34,35) ,"220126" = c(3,6,34,35) )
plot.ids = list( "220125" = c(3,6,12,15,17,23,19,34,35) 
      ) # Out of the traces in the >genotype<.#.csv files, which do you want to plot in the average?
genotype = "xxx" # something else? change here 
leaf = "L13" # "L08"
#####
setwd(home)
data = NULL
...
ggplot(na.omit(meanOnly), aes(x = Seconds))+
  ...
  My_Theme
ggsave(paste("Mean-",genotype,"-",l,"_curated.pdf",sep = ""), width = 6, height = 5, dpi = 300 )
