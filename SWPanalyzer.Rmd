...
## things to do
- change V to mV in the column name.
...
# to find half max frame, substract the halfMax V from the trace and pick the closest to 0.
  ...
  halfRepolframe =  max( which( abs(repolTrace-halfRepol) <= min(abs(repolTrace-halfRepol))+1 ) )   + depolFrame - stimFrame
    ...
    #which( abs(repolTrace-halfRepol) == min (abs(repolTrace-halfRepol))  )    + depolFrame - stimFrame # done max instead of min because of the oscillations that could happen while the voltage stabilizes.
...
```{r}
foldername = "gk.sk-230417-21"  # Replace the text in "" with the name you want your folder to have.
