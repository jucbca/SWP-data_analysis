...
The ending of the wounding event has to be marked with the F1 key.
Different parameters are quantified from wounded and receiving leaf.

Paremeters calculated are: 

Hyperpolarization phase
  hyperpol.slope          - Maximum slope in the first hyperpol phase 
  hyperpol.amplitude      - Hyperpolarization amplitude from first baseline to the peak of hyperpol 
  hyperpol.duration       - Duration of first hyperpolarization from time @ hyperpol.slope to time @ depol.slope
  
Depolarization phase
  depol.slope             - Maximum slope in the depolarization phase. This is a negative value. 
  ...
  depol.duration          - Time between the following two time points: 
                              ...
                              II) @ time at half maximum voltage between peak depolarization and peak repolarization 
...
Repolarization phase                            
  ...
  bl.2nd                  - Final baseline after SWP is finished
    ...
    INPUT:
Data for one experiment regardless of the amount of genotypes analyzed.
...
Follow these steps the first time you use this script:
  1. Click on create a project on the top left of this window to create a new project. Select the the option "Create new Directory".
  This will create a folder with a file with the same name, min that folder all the input and output will be located.
  
  This window will probably close and a new space will be created. Open this code again in that space.

  2. In the "gear" configurations button at the top of the window select "Chunk output in console" instead of "Chunk output inline". 
  ...
  4. Run the chunks one by one and in order (see below how to do that). If ran successfully, each chunk will indicate you to go 
  to the next.
  ...
  Note: The analysis is divided in 4 sections, each with multiple chunks. Run them in order and one by one. 
        ...
        later, do it after finishing one full section.  
...
DataExtraction <- function(data, date){ 
  ...
  # determine the leaf 1 or 2 identity (8, 13)
    if( L08.exist == TRUE && L13.exist == FALSE ) {
        ...
        info1 = names(data)[  grep( "08" , names(data) )  ]
    }
...
findSWPvalues <- function(trace, stimulus.time, sample.rate = 1){
  ...
  hyperpolFrame =  which(trace == maxHyperpol) - stimFrame # Find frame at maximum hyperpol
  ...
  depol.slope.frame = min( which(depoltrace.firstderivative == depol.slope) )+hyperpolFrame-stimFrame
...
# to find half max frame, substract the halfMax V from the trace and pick the closest to 0.
  ...
  # focus on the chunk between max hyperpol and max depol
  check.behind = 20
  hyperpolTrace = trace[ (hyperpolFrame-check.behind+stimFrame) : (hyperpolFrame+stimFrame+2)] # checks the trace 20s before the peak hyperpol.
...
#### filter to get to one single plant
  ...
  ### FUNCTION TO CALCUATE PARAMETERS         
          SWPanalysis = findSWPvalues(trace$V, 
                                      trace$Seconds[ max(which(trace$DayTime=="F1"))] )
          trace = na.omit(trace)
...
My_Theme =  theme(
  ...
  plot.title = element_text(color = "black", size = 20, angle = 0, hjust = 0.5, vjust = 0, face = "plain"), 
...
time = data.frame("Seconds"= c(0:590) )
...
for(i in 1:length(unique(data.plot$id)) ){
  ...
  vector.i = vector.i [(align.point-90):(align.point+500)]
