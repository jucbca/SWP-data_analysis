...
The wounding moment has to be marked with the F1 key. ??(more presice, we mark the beginning and ending of the wounding process)
Different parameters are quantified from wounded and receiving leaf.

Paremeters calculated at: (at oder are??)

Hyperpolarization phase
  hyperpol.slope          - Maximum peak of slope in the first hyperpol phase  (can we call it maximum slope?) 
  hyperpol.amplitude      - Hyperpolarization amplitude from first baseline to peak (of) hyperpol 
  hyperpol.duration       - Duration of first hyperpolarization from time @ hyperpol.slope to time @ (???)
  
Depolarization phase
  depol.slope             - Minimum peak of slope in the depolarization phase (I only know minimum and maximum slope, I know what you mean but is that the official term?)
  depol.amplitude         - Depolarization amplitude from hyperpol peak to depol peak
  depol.duration          - Time between the following two time points: 
                              I) @ time of depol.slope
                              II) @ time of middle voltage between peak repolarization and peak (what do you mean here?)

Repolarization phase                            
  hyperpol.amplitude.2nd  - Repolarization amplitude from second baseline to peak repolarization.
  bl.2nd                  - Final baseline after SWP is finished
    ...
    INPUT:
Data for one experiment (one day of recordings) regardless of the amount of genotypes analyzed. (This suggests that one experiment is only one day of reacordings, but it can be several days of recordings. What you say in the next sentence) All the traces 
from plants that grew together and were recorded together are considered to be one experiment.  ( think this information can go into the manuscriptand does not need to be repeated again. What do you think?)
...
Write all the information about the experiment in the leaf 8 label. 
Follow this structure for the names of the columns in the .txt file.
        Leaf 8 --         "Ez_Lyy.xPG_w" 
        Leaf 13--         "Ez_Lyy.x"
...
This is a guide, ABSOLUTELY re-run these tests, or other more suited to your data, manually with the data provided in point 4.
...
Traces are filtered with a Savitzky-Golay method which reduces the amplitude (by around 5mV) of traces 
with a very sharp spike in the peak depolarization. 
If there is a phenotype at very sharp depolarization, we are probably missing it.
...
Follow these steps the first time you use this script:
  1. Click on create a project on the top left of this window to create a new project. 
  This will create a folder that will be your "home" directory. 

  2. In the "gear" button at the top of the window select "Chunk output in console" instead of "Chunk output inline". 
  This will show all the steps of the process properly in the console.
  
  3. Paste a copy of the exported folders with the .txt data from one batch of experiments into the home folder you 
  just created (use format described above in the INPUT section). 
  
  4. Run the chunks in order (see below how to do that). If ran successfully, each chunk will indicate you to go 
  to the next.
  
  Note: The analysis is divided in 4 sections, each with multiple chunks. Run them in order. 
        There is a checkpoint between sections; this means that it if necessary to close the program and continue
        later, do it after finishing one full section (3.1, 3.2, and 3.3 in 3).  
        REMEMBER to run section 1 every time you open the program, even if you are not starting from scratch.
...
2.1. Write your folder's name in the foldername variable. 
     Wrire your control group in the controlgroup variable. This is the control condition Example. 
...
2.2. Run this to extract the data and save it in a folder with the name previously given. Ignore the errors that are shown at the end. This is due to the F1 marks in the traces. Check that you have the >genotype<.csv files in the folder that you created in the last step and continue with step 3.

If this stops with an error instead of a success message, the last file printed in the console does not have the proper formating. Check that column names fit the INPUT conditions above.
...
- Run as many times as required until the step 3.3 tells you are done. 
This should run through all traces in the ideal case, but will stop in case some trace is damaged. 
The easiest thing to do is to re-run this chunk and skip/erase the trace when the program first asks you what to do. 

output - 
  a >genotype< folder with figures of the analyzed traces for both leaves available. 
  Figures names are "#.Lxx.pdf", where xx is the leaf (08 or 13). 
   where: # is the plant number, unique in the batch and consistent over all files generated. 
...
output - Tells you if you are done with all the traces or if you need to keep running step 3.2
```{r}
