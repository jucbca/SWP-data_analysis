...
Repolarization phase                            
  hyperpol.amplitude.2nd  - Amplitude from second baseline to peak repolarization.
...
A folder with .txt files having the following structure:
 - Folder named: "YYMMDD"
 - Raw data should be in .txt files, as exported from LabScribe

In LabScribe name the electrodes (= column name) used in the recording with the following structure.
...
If there is a phenotype at very sharp depolarization, we are probably missing it. (I would leave this out)
...
Follow these steps the first time you use this script:
  ...
  Note: The analysis is divided in 4 sections, each with multiple chunks. Run them in order. 
        There is a checkpoint between sections; this means that if necessary to close the program and continue
...
If you don't have the packages installed in your computer does it by typing in the console:
...
Create a folder with the name of your (s)election. Here all the output will be saved.
...
plot.ids = list( "230228" = c(1,3,4,5,7,8,9,10,11,14,15,16,17,18,19,20),
                 "231005" = c(2,3,6,7,8,9,10,13,14,15,16,18),
