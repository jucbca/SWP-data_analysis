...
for( f in list.files()[grep(".txt",list.files())] ){ # 
  ...
  #### ACA!  segment both electrodes an put in same list!
...
input   -  >###ms.RData< traces divided by duration and electrode.
...
for( f in grep("RData",list.files()) ){ # go through the lists of the different experiment
  ...
  for(i in 1:length(trace.segmented) ) { # loop through the stim times
    ...
    for(j in 1:(length(trace.segmented[[i]])-1) ){
      ...
      one.stimulus = cbind( "stim.duration-ms-" =  as.numeric( strsplit(names(trace.segmented)[i],split = " ")[[1]][1] )  ,
                          ...
                          data.frame( CalculateParameters( trace.segmented[[i]][[j]] ) )
      )
...
ggplot(dplyr::filter(datasummary, electrode==2), aes(`stim.duration-ms-`, `peak.depol`, color = `electrode` )) +
  geom_point(size = 3) +
  ...
  ylab ("Depolarization amplitude mV") + 
  ggtitle("Dose-response to stimulus duration. Power-1") +
  My_Theme

ggsave(paste(batchfolder,"DoseResponse-time.pdf",sep = "/"))
setwd(home)
```
