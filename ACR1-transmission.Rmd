...
Output  - List >savename.RData< with all the separated pulses of all for that recording
...
for( f in list.files()[grep(".txt",list.files())] ){ # 
  ...
  trace$Stim = stimulus
  trace$Leaf = leaf
  trace$id = recording.id # esto de donde sale??
  PlotTrace(trace)
  # Filter?
  ...
  trace.segmented = segment.stimuli(trace, stimdelay = 200)
...
4'. Dose-Response (time vs amplitude) Calculate parameters
input   -  >savename.RData< traces divided by duration and electrode.
output  - datasummary.csv file with all the parameters for each stimulus
```{r}
setwd(batchfolder)
datasummary = NULL
...
for( f in grep("RData",list.files()) ){ # go through the lists of the different experiment
  print(list.files()[f])
  load(list.files()[f])
  savename = list.files()[f] %>% substr(1,nchar(list.files()[f])-6)
  for(i in 1:length(trace.segmented) ) { # loop through the stim times
    print(names(trace.segmented)[i])
    print( as.numeric( strsplit(names(trace.segmented)[i],split = " ")[[1]][1] ) )
    for(j in 1:length(trace.segmented[[i]]) ){
      print( names(trace.segmented[[i]][j]) )

      one.stimulus = cbind( "stim.duration.ms." =  as.numeric( strsplit(names(trace.segmented)[i],split = " ")[[1]][1] )  ,
                          "electrode" = names(trace.segmented[[i]])[j],
                          "stimulus" = trace.segmented[[i]][[j]]$Stim[1] , 
                          data.frame( CalculateParameters( trace.segmented[[i]][[j]],stimdelay=200) ),
                          "genotype" = trace.segmented[[i]][[j]]$Genotype[1]
      )
      plot(trace.segmented[[i]][[j]]$Time[one.stimulus$stimon:one.stimulus$stimoff], trace.segmented[[i]][[j]]$mV[one.stimulus$stimon:one.stimulus$stimoff],
           main = paste(savename,"_",names(trace.segmented)[i],"_electrode",j, sep = "" )   )
      
      plot.title = paste(savename,"_",
                         one.stimulus$stim.duration.ms.[1],"ms_",
                         one.stimulus$leaf[1],"_",
                         "electrode",j,
                         sep = "" )
      Sys.sleep(0.5)
      pdf(file= paste(batchfolder,"/PulsePlots/",plot.title,".pdf",sep=""))
      plot(trace.segmented[[i]][[j]]$Time, trace.segmented[[i]][[j]]$mV,
           main = plot.title ,
           cex=0.5   ) +
        abline(h=one.stimulus$peak.depol, col="red") +
        abline(v=one.stimulus$peakdepol.time, col="red", lty=2) +
        abline(v=one.stimulus$tau.abstime, col="blue", lty=2) +
        abline(v=c(trace.segmented[[i]][[j]]$Time[one.stimulus$stimon],
                   trace.segmented[[i]][[j]]$Time[one.stimulus$stimoff]), 
               col="black") 
        dev.off()
      
      
      datasummary = rbind(datasummary, one.stimulus)
    }
  }
}
datasummary$position = "Distal"
for( r in 1:nrow(datasummary)){
    if( length(grep(datasummary$electrode[r],datasummary$stimulus[r])) == 1 ){
    datasummary$position[r] = "Local"
  }
}
```
...
5'. Plot the dose response curve from the datasummary.csv file from 4'
...
ggplot(dplyr::filter(datasummary), aes(`stim.duration.ms.`, `peak.depol`, color = `position` )) +
  geom_point(size = 3) +
  scale_x_continuous(trans='log10',
                     breaks=c(1,10,100,1000,10000) #trans_breaks('log10', function(x) 10^x),
                     ) +
  #xlim(c(1,5)) +
  xlab ("Stimulus duration ms - log10 scale") + 
  ylab ("Peak depol (mV)") + 
  ggtitle("Dose-response to stimulus duration. Power-1") +
  My_Theme

ggsave(paste(batchfolder,"DoseResponse-peak.depol.pdf",sep = "/"))
setwd(home)
```
...
for(g in unique(datasummary$genotype) ){
    for(v in names(datasummary)[names(datasummary) %in% parameters ] ){
      local.data = datasummary %>% dplyr::filter(genotype == g & 
                                    position == "Local") %>%
        select(all_of(v))
      
      local.data = na.omit(local.data[,1])  
      distal.data = datasummary %>% dplyr::filter(genotype == g & 
                                    position == "Distal") %>%
        select(all_of(v))
...
ggplot(dplyr::filter(datasummary, `position` == "Local"), #`stim.duration.ms.`>150
       aes(`stimulus`, `peak.depol.slope`, color = `stim.duration.ms.`)) +
  geom_violin(size = 1) +
  ...
  ylab ("tau (s)") + 
  ggtitle("535nm stimulus on Col-0:ACR1") +
  My_Theme
ggsave("Propagation-tau.vs.position.pdf")



wilcox.test(  datasummary %>% dplyr::filter(genotype == g & 
                                    stimulus == "S1") %>%
        select(all_of("tau")),
             datasummary %>% dplyr::filter(genotype == g & 
                                        stimulus == "S2") %>%
            select(all_of("tau"))
...
dataset1 = datasummary %>% dplyr::filter(position == "Local" & 
                                    stimulus == "S1") %>%
        select(all_of("tau"))

dataset2 =  datasummary %>% dplyr::filter(position == "Local"& 
                                        stimulus == "S2") %>%
            select(all_of("tau"))

dataset1 = na.omit(dataset1[,1])
dataset2 = na.omit(dataset2[,1])
shapiro.test(dataset2)$p %>% round(3)

test = data.frame( "p"= round(wilcox.test(dataset1, dataset2)$p.value,3),
                   "W"= wilcox.test(dataset1, dataset2)$statistic
)
write.csv(file = "tau.vs.position-WM.csv", test)
```
